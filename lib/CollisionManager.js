...
export class CollisionManager {
    ...
    doKnockOnCollsion(x, y, collider, levels) {
        if (levels == 0) {
            return; 
        }
        for(let key of collider.validCollisionIds) {
            let nearCollider = null;
            for (let i = 0; i < Collider.all.length; i++) {
                if (Collider.all[i].id == key){
                    nearCollider = Collider.all[i];
                }
            }
            if (nearCollider != null) {
                if (nearCollider.id != collider.id && !nearCollider.static && this.collidersOverlap(collider, nearCollider)) {
                    nearCollider.x += x;
                    nearCollider.y += y;
                    this.doKnockOnCollsion(x, y, nearCollider, levels+1);
                }
            }
        }
    }
    ...
    doCollideCircleToCircle(collider1, collider2) {
        ...
        if (collider1.static == false) {
            ...
            let normalVector = CollisionUtilities.normalize(vectorBetween.x, vectorBetween.y);
            let bounce = collider1.bounciness/100;

            collider1.velocity.x = collider1.velocity.x * (Math.max(1 - Math.abs(normalVector.x), bounce));
            collider1.velocity.y = collider1.velocity.y * (Math.max(1 - Math.abs(normalVector.y), bounce));
        }
        if (collider2.static == false) {
            ...
            let normalVector = CollisionUtilities.normalize(vectorBetween.x, vectorBetween.y);
            let bounce = collider2.bounciness/100;

            collider2.velocity.x = collider2.velocity.x * (Math.max(1 - Math.abs(normalVector.x), bounce));
            collider2.velocity.y = collider2.velocity.y * (Math.max(1 - Math.abs(normalVector.y), bounce));
        }
        ...
        if (collider1.static == false) {
            ...
            collider1.x += x;
            collider1.y += y;
            this.doKnockOnCollsion(x, y, collider1, 0);
        }
        if (collider2.static == false) {
            ...
            collider2.x += x;
            collider2.y += y;
            this.doKnockOnCollsion(x, y, collider2, 0);
        }
    }
    ...
    doCollideCircleToBox(collider1, collider2) {
        ...
        let boxBounce = boxCollider.bounciness/100;
        boxPercentage = boxPercentage * boxBounce;
        ...
        if (boxCollider.static == false) {
            ...
            boxCollider.velocity.x = boxCollider.velocity.x * (Math.max(1 - Math.abs(normalVector.x), boxBounce));
            boxCollider.velocity.y = boxCollider.velocity.y * (Math.max(1 - Math.abs(normalVector.y), boxBounce));
        ...
        if (circleCollider.static == false) {
            ...
            let bounce = circleCollider.bounciness/100;

            circleCollider.velocity.x = circleCollider.velocity.x * (Math.max(1 - Math.abs(normalVector.x), bounce));
            circleCollider.velocity.y = circleCollider.velocity.y * (Math.max(1 - Math.abs(normalVector.y), bounce));
        }
        ...
        if (boxCollider.static == false) {
            ...
            boxCollider.x += x;
            boxCollider.y += y;
            this.doKnockOnCollsion(x, y, boxCollider, 0);
        }
        if (circleCollider.static == false) {
            ...
            circleCollider.x += x;
            circleCollider.y += y;
            this.doKnockOnCollsion(x, y, circleCollider, 0);
        }
    }
    ...
    doCollideBoxToBox(collider1, collider2) {
        ...
        if (intersect == null) { //circle entirely within rectangle
